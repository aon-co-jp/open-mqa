...
impl Verify for StreamInfo {
    fn verify(&self) -> Result<(), VerifyError> {
        ...
        // open-mqa patch: FLAC spec allows up to 655,350 Hz in frame headers (1,048,575 in STREAMINFO); upstream capped at 96 kHz.
        verify_range!("sample_rate", self.sample_rate(), ..=655_350)?;
